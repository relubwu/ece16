...
if write_flag:
    write_filename = "pedometer_training_active.csv"
...
def update_data():
    ...
    if (time_end - time_start >= 1):
        time_start = time()
        bpm = hr_calc.process(data_buffer[0],data_buffer[2])
        if (bpm[1] is not None):
            print("BPM: %.2f" % bpm[1])

    return [(data_buffer[0], data_buffer[2]), (data_buffer[0], data_buffer[4])]  # Plot t, imu data, filtered imu data
