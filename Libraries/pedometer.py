...
class Pedometer:
    ...
    def train(self, train_file_active, train_file_inactive):
        ...
        peaks, properties = find_peaks(data_active, prominence=400, width=4)
        plt.figure()
        plt.plot(data_active)
        #plt.vlines(x=peaks, ymin=data_active[peaks] - properties["prominences"], max = data_active[peaks], color = "C1")
        plt.hlines(y=properties["width_heights"], xmin=properties["left_ips"], xmax=properties["right_ips"], color = "C1")

        plt.xlabel("X")
        plt.ylabel("Y")
        plt.title("Raw Data")
        plt.plot(peaks, data_active[peaks], "x")
        plt.show()
        ...
        print("Pedometer KNN Model Accuracy:", accuracy)
